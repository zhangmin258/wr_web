$(function(){
	...
	$.ajax({
		type:"post",
		url:"/scoreexchange/searchuserinfo",
		...
		success:function(data){
			...
			if( data.userMini.UserImg.length>25 ){
				$("#userLogo").attr("src",data.userMini.UserImg);
			}
			var wh;
			setTimeout(function(){
				if( $("#userLogo").width()<$("#userLogo").height() ){
					wh=$("#userLogo").height()/$("#userLogo").width();
					$("#userLogo").css({width:"2.48rem",height:"auto","marginTop":"-"+0.32*wh+"rem"});	
				}else if( $("#userLogo").width()>$("#userLogo").height() ){
					wh=$("#userLogo").width()/$("#userLogo").height();
					$("#userLogo").css({width:"auto",height:"2.48rem","marginLeft":"-"+0.32*wh+"rem"});
				}else{
					$("#userLogo").css({width:"2.48rem"});
				}
			},100);
		}
	});
