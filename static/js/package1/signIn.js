$(function(){
	...
	$('#goRemind').on("click",function(){
		if( isRemind==false ){
			isRemind=true;
			$(this).attr("src","../../static/img/package1/alertOpen.png");
			tuiter(1);
		}else{
			isRemind=false;
			$(this).attr("src","../../static/img/package1/alertClose.png");
			tuiter(0);
		}
	});
	...
	$.ajax({
		...
		success:function(data){
			...
			if( data.RemindStatus==0 ){
				isRemind=false;
				$("#goRemind").attr("src","../../static/img/package1/alertClose.png");
			}else{
				isRemind=true;
				$("#goRemind").attr("src","../../static/img/package1/alertOpen.png");
			}
			if( data.status==1 ){
				$("#signIn").attr("src","../../static/img/package1/signIned.png");
			...
			$.each(data.SignReward,function(index){
				if( data.SignReward[index].IsReceive==0 ){
					//未领取
					$("#navBtn div").eq(index).find('img').attr("src","../../static/img/package1/"+data.SignReward[index].SignCount+"CanGet.png");
				}else if( data.SignReward[index].IsReceive==1 ){
					//已领取
					$("#navBtn div").eq(index).find('img').attr("src","../../static/img/package1/"+data.SignReward[index].SignCount+"haveGet.png");
				}else if( data.SignReward[index].IsReceive==2 ){
					//未完成
					$("#navBtn div").eq(index).find('img').attr("src","../../static/img/package1/"+data.SignReward[index].SignCount+"Can'tGet.png");
				}
		...
		$.ajax({
			...
			success:function(data){
				if( data.ret==200 ){
					...
					queryPeas();
					$(that).find('img').attr("src","../../static/img/package1/"+thisName+"haveGet.png");
				}else{
					window.postMessage("Alert+"+data.msg,'*');
				}
			}
