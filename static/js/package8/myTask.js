		...
		success:function(data){
			...
			$.each(data.DailyMission,function(index){
				var liStr="<li>"+
					...
					"<p class='addPeas'>"+
						...
						"<img src='../../static/img/package8/peasYellow.png' />"+
					"</p>"+
			...
			for( var i=0; i<$(".comStatic span").length; i++ ){
				...
				if( getStatus==0 ){
					...
					$(".comStatic span").eq(i).parent().parent().find('button').css("background","#ff8c27");
				}else if( getStatus==1 ){
					...
					$(".comStatic span").eq(i).parent().parent().find('button').css("background","#ffb353");
				}else if( getStatus==3 ){
